 ...
 * (GET /public/tours/popular-destinations). No hardcoded destinations —
 * the old placeholder list linked to /tours/goa-style pages that had no
 * content. Cards now deep-link into the tours listing pre-filtered by
 * destination, which always renders real results. Empty DB → Coming Soon.
 */
            ...
            <MotionStaggerItem key={d.city_id} className="h-full">
              <MotionGlow color="primary" intensity={0.2} size={240} className="h-full rounded-2xl">
                <MotionTilt max={3} className="h-full">
                  <a
                    href={`/tours?destination=${encodeURIComponent(d.city_name)}`}
                  ...
                  >
                    ...
                    {d.hero_image_url && (
                      <div
                        ...
                        style={{ backgroundImage: `url(${d.hero_image_url})` }}
                      />
                    )}
                    ...
                    {d.package_count > 0 && (
                      <span className="absolute top-3 right-3 z-10 inline-flex items-center gap-1 px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-white/90 backdrop-blur text-primary-700">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-4 text-white">
                      <div className="text-xs text-white/70 mb-1 inline-flex items-center gap-1">
                        <MapPin className="h-3 w-3" /> {d.state_name ?? "India"}
                      </div>
                      <h3 className="text-lg font-bold mb-1 group-hover:text-accent-200 transition-colors">{d.city_name}</h3>
                      {d.starting_price != null && Number(d.starting_price) > 0 && (
                        <div className="text-xs text-white/80">
                          From <span className="font-bold text-white">₹{Number(d.starting_price).toLocaleString("en-IN")}</span>
                        </div>
                      )}
                    </div>
                  </a>
